...
class UserController {
  async login(req, res) {
    ...
    try {
      ...
      if (!email || !password) {
        return httpResponse.badRequest("Erro de login: É necessário inserir um e-mail e uma senha válidos.");
      }

      // Verifica se existe uma conta associada ao email fornecido
      const userExists = await database.users.findOne({ where: { email } });
      if (!userExists) {
        return httpResponse.notFound("Não há uma conta com esse endereço de email.");
      }

      // Compara a senha fornecida com a senha armazenada no banco de dados
      const isPasswordValid = await bcrypt.compare(password, userExists.password);
      if (!isPasswordValid) {
        return httpResponse.badRequest("A senha está incorreta! Tente novamente.");
      }

      // Gera um token de acesso para o usuário logado
      const accessToken = jwt.sign({ id: userExists.id }, process.env.TOKEN_SECRET, { expiresIn: process.env.TOKEN_EXPIRES_IN });
  ...
  async getById(req, res) {
    ...
    try {
      ...
      if (!user) return httpResponse.notFound("Usuário não encontrado! Verifique se o ID é válido.");
  ...
  async getProfile(req, res) {
    ...
    try {
      ...
      const loggedUser = await database.users.findByPk(loggedUserId, {
        include: [{ model: database.roles }],
      });

      if (!loggedUser) {
        return httpResponse.notFound("Usuário logado não encontrado!");
      }

      console.log("Rota /user/loggedInfo chamada com sucesso.");
      console.log("Informações do usuário logado:", req.userId);

      return httpResponse.ok({ loggedUser });
    } catch (error) {
      console.error("Erro em getLoggedUserInfo:", error);
      return httpResponse.internalError(error);
    }
  }
  ...
  async create(req, res) {
    ...
    try {
      ...
      if (typeof name !== "string" || name.trim() === "" || !email || !password || !roleId) {
        return httpResponse.badRequest("Por favor, preencha todos os campos obrigatórios!");
      }
      ...
      if (userAlreadyExists) {
        return httpResponse.badRequest("Já existe uma conta com este endereço de email.");
      }
  ...
  async update(req, res) {
    ...
    try {
      ...
      if (!id) {
        return httpResponse.badRequest("Parâmetros inválidos! Certifique-se de fornecer o ID do usuário.");
      }

      const userToUpdate = {};
      const userExists = await database.users.findByPk(id);
      if (!userExists) {
        return httpResponse.notFound("Usuário não encontrado. Verifique se o ID é válido.");
      }
      ...
      if (password) {
        ...
        if (/\s/.test(password)) {
          return httpResponse.badRequest("A senha não pode conter espaços em branco.");
        }
        const passwordHashed = await bcrypt.hash(password, Number(process.env.SALT));
